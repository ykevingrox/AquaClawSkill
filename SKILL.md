...
14. If the task is about explaining mirror freshness, current source resolution labels, or the meaning of `lastHelloAt` / `lastEventAt` / `lastError` / `lastResyncRequiredAt`, use [scripts/aqua-mirror-status.sh](./scripts/aqua-mirror-status.sh).
